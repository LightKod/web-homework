...
export default function TaskList() {
    const { tasks, tasksDispatch, filter } = useContext(AppContext)!
                    ...
                    <div key={index} className={clsx(
                        ...
                        <div className="grid grid-cols-[6fr_4fr_1fr] gap-2 items-center grow p-4 bg-white rounded-md shadow-md relative overflow-hidden">
                            ...
                            <div className="flex justify-end">
                                <button onClick={() => tasksDispatch({ type: "DELETE_TASK", payload: task.id })} className="text-gray-950 hover:text-red-600 transition-all duration-300">
                                    <TrashIcon className="w-5"/>
                                </button>
                            </div>
                        </div>
                        {task.completed ? (
                            <button type="button" onClick={() => tasksDispatch({ type: "TOGGLE_TASK_COMPLETION", payload: task.id })} className="flex items-center justify-center h-full min-w-[4rem] shrink-0 p-4 md:min-w-[5.75rem]">
                                <div className="w-full h-full bg-contain bg-no-repeat bg-center bg-[url('/complete_mark.png')]"/>
                            </button>
                        ) : (
                            <button type="button" onClick={() => tasksDispatch({ type: "TOGGLE_TASK_COMPLETION", payload: task.id })} className="flex items-center justify-center h-full min-w-[4rem] shrink-0 p-4 bg-white rounded-md shadow-md text-gray-950 hover:text-violet-800 md:min-w-[5.75rem] transition-all duration-300">
